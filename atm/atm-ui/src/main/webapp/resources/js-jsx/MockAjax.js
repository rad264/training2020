...
$.ajax = function (config) {
    var resources = {
        ...
        "/atm-api/accounts/{accountNumber}/deposits": {
            POST: function (params, data) {
                var balance =
                    accountBalances[params.accountNumber] + parseFloat(data);
                if (balance)
                    success({
                        balance: balance,
                    });
        ...
        "/atm-api/accounts/{accountNumber}/transactions": {
            GET: function (params) {
                ...
                if (transactions) success(transactions);
                else error(404);
            },
        },
    };
            ...
            if (operations[config.type]) {
                ...
                for (var i in staticUrlParts) {
                    if (staticUrlParts[i].match(/^\{[^\/]*\}$/)) {
                        var paramName = staticUrlParts[i].substring(
                            1,
                            staticUrlParts[i].length - 1
                        );
